...
def import_data(bot: Bot, update):
	...
	if msg.reply_to_message and msg.reply_to_message.document:
		...
		try:
			...
			return msg.reply_text("There is problem while importing the data! Please ask in @BotSupportGroup about why this happened.")
		# Check if backup is from self
		try:
			if str(bot.id) != str(data[str(chat.id)]['bot']):
				return msg.reply_text("Backup from another bot that is not suggested might cause the problem, documents, photos, videos, audios, records might not work as it should be. However, You can still request a feature regarding this in @BotSupportGroup !")
		except:
			pass
		# Select data source
		...
		try:
			...
			msg.reply_text("An error occurred while recovering your data. The process failed. If you experience a problem with this, please ask in @BotSupportGroup . My owner and community will be happy to help. Also, bugs report makes me even better!\nThank you!")
...
def export_data(bot: Bot, update: Update, chat_data):
	...
	f=open("tg_botb{}.backup".format(chat_id), "w")
	...
	try:
		...
		pass
	bot.sendDocument(current_chat_id, document=open('tg_botb{}.backup'.format(chat_id), 'rb'), caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `ShaktiMaan-Backup` is specially made for notes.".format(chat.title, chat_id, tgl), timeout=360, reply_to_message_id=msg.message_id, parse_mode=ParseMode.MARKDOWN)
	os.remove("tg_botb{}.backup".format(chat_id)) # Cleaning file
